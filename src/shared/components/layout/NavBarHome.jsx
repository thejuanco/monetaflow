...
export default function NavBarHome() {
  ...
  return (
    <nav className="w-full sticky top-0 z-50 border-b border-gray-300 backdrop-blur">
      <div className="flex justify-between h-14 items-center px-4 md:px-8">
        ...
        <div className={`md:flex md:space-x-4 ${menuOpen ? 'block' : 'hidden'} absolute md:static top-16 right-6 md:top-0 bg-gray-100 md:bg-transparent rounded-lg p-4 md:p-0 shadow-lg md:shadow-none`}>
          <nav className="flex items-center space-x-4 text-sm font-medium">
            ...
            <Link to="/auth/signin" className="flex items-center bg-gray-800 text-white font-medium py-2 px-4 rounded-full hover:bg-gray-900 transition-transform duration-300 transform hover:scale-105">
              Iniciar Sesión
            </Link>
          </nav>
        </div>
